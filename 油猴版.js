...
(function () {
  'use strict';
    ...
    var pages = 5; // 初始检查做题记录页数，建议设置为 1-2
                ...
                .then(content => {
                    ...
                    for (var i = Math.min(content.currentData.records.result.length - 1, 19); i >= 0; i--) {
                        if (content.currentData.records.result[i].id > lst[cnt]) {
                            ...
                            var html = '<tr><td>' + user + "</td><td><b><a href='https://www.luogu.com.cn/problem/"+pid+"'>"+ pid + '</a></b></td><td>' +"<b><a style='color:"+ cl + "' href='https://www.luogu.com.cn/record/"+content.currentData.records.result[i].id+"'>"+sts+"</a></b></td> " + "<td><a style='color:" + col + "' href='https://www.luogu.com.cn/problem/" + pid + "' target='_blank'>" + title + "</a></td><td>" + new Date(time * 1000).toLocaleString() + "</td></tr>";
    ...
    window.setInterval(function () {
        ...
        let html = '<tr><th>用户</th><th>题号</th><th>状态</th><th>标题</th><th>时间</th></tr>';
